    ...
    , ui(new Ui::LecteurVue)
{
    ...
    QObject::connect(ui->a_changerImage,SIGNAL(triggered()),this,SLOT(changerTitreImage()));
    ...
    ui->a_changerImage->setEnabled(false);
...
void LecteurVue::chargerDiapo()
{
    ...
    ui->a_changerImage->setEnabled(true);
    ...
    //Récupérer le titre du diaporama
    qDebug()<< "changement du diaporama";
    _nomDiapo = L->getTitreDiapo();
    qDebug()<< _nomDiapo;
    ...
    ui->l_mode->setText("manuel");
    ui->l_titre->setText(titreImage);
    ui->l_titreDiapo->setText(_nomDiapo);
...
{
    QMessageBox::information(this, "A propos", "Bonjour, ceci est la version 6 du lecteur de diaporama,"
                                               "créée le 05/06/2023 par Dylan Victoras, Yannis Duvignau et Nicolas Conguisti");
}

void LecteurVue::changerTitreImage()
{
    qDebug()<< "changement du nom de l'image";
    Dialog_titreImage *Dlg_titre_image = new Dialog_titreImage;
    Dlg_titre_image->exec();

    //Si le nom de l'image a bien été changé, on le modifie dans la base de données
    _changement = Dlg_titre_image->getChangement();

    if(_changement)
    {
        _nomImage = Dlg_titre_image->getnomImage();

        qDebug() << "Nom de l'image changé à " + _nomImage;

        //Récupération de idPhoto

        int idPhoto = L->imageCourante()->getIdPhoto();
        ...
        QString sqlQuery = "UPDATE Diapos "
                           "SET titrePhoto = :nomImage "
                           "WHERE idphoto = :idphoto;";

        QSqlQuery query;
        query.prepare(sqlQuery);
        query.bindValue(":nomImage", _nomImage);
        query.bindValue(":idphoto", idPhoto);
